...
- Если нет такого пользователя, или нет такой группы, или нет пользователя в группе, возвращает JSON объект с полем error равным сообщению об ошибке, код возврата `400`.
```url
Пример:
