...
## POST /group/list_admins - получить список администраторов группы
- Принимает JSON-объект с полем `group_id` - ID группы, администраторов которой нужно вывести.
- Возвращает JSON-объект соответствия между ID администратора группы и его именем для всех пользователей сервиса.
