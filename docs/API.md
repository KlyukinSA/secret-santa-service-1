...
- Принимает JSON объект с полями `user_id` равнному требуемому id нужного пользователя и `group_id` равному требуемому id нужной группы. 
- Возвращает JSON объект с полем `id` равным ID тайного санты для заданного пользователя в случае успеха, код возврата `200`.
- Если введены неверные данные - возвращает JSON объект с полем `error` равным сообщению об ошибке, код возврата `400`.
- Если входные данные неверного формата (`{"user":"4"}, {"group":"3"}`), то сервер паникует, а клиент не получает ответ на свой запрос.
- порядок ввода `user_id` и `group_id` не важен и приведёт к одинаковому результату.
```json
{
  "group_id":"1",
  "user_id":"2",
