...
- Принимает json объект с полем name равным требуемому имени нового пользователя. Возвращает json объект с полем id равным айдишнику нового пользователя в случае успеха, код возврата 200.
...
- В случае успеха возвращает json-объект с именем нового администратора, код возврата 200.
- При некорректных данных возвращается json-объект с полем error, равным сообщению об ошибке, код возврата 400.
...
- Запускает секретного санту в группе с group_id, если admin_id это id администратора группы group_id. 
- Секретный Кыш Бабай: 1.) Закрыть группу 2.) Выставить всем участникам группы santa_id, в котором написан id того пользователя, для кого в группе они стали секретным Кыш Бабаем.
...
- Для пользователя user_id в группе group_id, возвращает id того пользователя, для кого в группе они стали секретным Кыш Бабаем.
```json
{
  "group_id":4,
  "user_id":3,
...
## GET /user/info - получить инфо по group_id
## GET /group/member_info - получить инфо о пользователе с member_id в group_id
## GET /group/info - получить инфо по group_id 
## POST /group/kick_user - удалить пользователя из группы по group_id, actor_id, user_id
## GET /group/list_members - получить список группы по group_id 
## GET /group/list_admins - получить список администраторов группы по group_id
